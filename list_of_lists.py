...
def total_enrollment(country_univs):
    total_students_enrolled = 0
    total_tuition_fees = 0
    students_enrolled = 0
    for row in range(len(country_univs)):
        for rowIndex in range(3):
            if rowIndex == 1:
                students_enrolled = country_univs[row][rowIndex]
                total_students_enrolled += students_enrolled
            elif rowIndex == 2:
                total_tuition_fees += students_enrolled * country_univs[row][rowIndex]
    return total_students_enrolled, total_tuition_fees
